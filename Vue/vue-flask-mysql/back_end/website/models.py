...
ENCRYPTION_KEY_AS_STRING = os.environ.get("ENCRYPTION_KEY_AS_STRING")
...
class User(db.Model):
    # values: ["retailer user", "ho brand User", "ho user", "super user"]
    __tablename__ = 'users'
    ...
    def is_user_authorised(user, crud_operation, new_values_dictionary=None):
        ...
        crud_authorities = dict(
            ...
            read = ["ho brand user", "ho user", "super user"],
    ...
    def get_dict(self):
        ...
        try:
            ...
            name = "not known"
        try:
            email = get_data_from_encrypted_data(self.email, ENCRYPTION_KEYS[self.pii_key_id])
        except:
            email = "not known"
 
        return_package = dict(id=self.id, created_timestamp=self.created_timestamp, name=name, email=email, is_deleted=self.is_deleted, failed_login_streak=self.failed_login_streak, pii_key_id=self.pii_key_id)
    ...
    def update_from_dictionary(instance, dictionary):
        ...
        try:
            ...
            if "email" in dictionary:
                instance.email = get_encrypted_data(dictionary["email"], latest_encryption_key)
            else:
                email = get_data_from_encrypted_data(instance.email, ENCRYPTION_KEYS[instance.pii_key_id])
                instance.email = get_encrypted_data(email, latest_encryption_key)
...
class Log(db.Model):
    ...
    def get_dict(self):
        ...
        try:
            ...
            support_email_sent = ""
        dictionary = dict(id=self.id,  created_timestamp=self.created_timestamp.strftime('%m/%d/%Y-%H:%M:%S'), support_email_sent =support_email_sent, logger=self.logger, level=self.level, trace=trace, msg=self.msg )
    ...
    def get_dict(self):
        ...
        return_package = dict(logger=self.logger, level=self.level, trace=self.trace, msg=msg, pii_key_id=self.pii_key_id)
